var Hash = require("hashish"), defaultRules = require("../rules"), step = require("step"), merge = require("batbelt").merge;
...
exports = module.exports = function(options) {
    ...
    function parseRule(ruleStr) {
        var regex = /^([^(]+)(\((.*)\))?$/, ruleGroup = regex.exec(ruleStr).slice(1);
    ...
    function parseParams(req, parseCb) {
        var rule, ruleFn, normalParam;

        step(function parseParams() {
            var self = this;
            ...
            config.parameters.forEach(function(paramDef) {
                function applyRules(oldValue, rules, cb) {
                    if (rules.length > 0) {
                        ...
                        require("due").mock(ruleFn).call(global, oldValue).then(function(err, newValue) {
                            if (err) {
    ...
    return function(req, res, next) {
        ...
        require("due").mock(parseParams).call(global, req).then(function(err) {
            var errors = [], error;
