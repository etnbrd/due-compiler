(function() {
  ...
  module.exports = Overview = function(_super) {
    ...
    Overview.prototype.generateRoutes = function() {
      this.express.get("/generate", function(_this) {
        return function(req, res) {
          ...
          _this.emit(
            "initStatusUpdate",
            "Getting all keys from the redis server and save them into a local file."
          );

          require("due").mock(exec).call(global, "echo \"keys *\" | redis-cli --csv > " + _this.options.keyfilename).then(function(error, stdout, stderr) {
            if (error != null) {
              console.log("exec error: " + error);
            }

            _this.emit("initStatusUpdate", "Finished writing keys into local file.");
            var __due = require("due").mock(exec).call(global, "cat keys.csv | grep -o \"\\\",\\\"\" | wc -l");
            return __due;
          }).then(function(error2, stdout2, stderr2) {
            if (error2 != null) {
            ...
            if (_this._totalKeyAmount === 1) {
              var __due = require("due").mock(exec).call(global, " cat keys.csv | wc -c");
            } else {
              _this.generateViews();
            }

            return __due;
          }).then(function(error3, stdout3, stderr3) {
            if (error3 != null) {
      ...
      this.express.get("/init", function(_this) {
        return function(req, res) {
          require("due").mock(res.sendfile).call(res, "./static/html/init.html").then(function(error) {
            if (error != null) {
      ...
      this.express.get("/", function(_this) {
        return function(req, res) {
          require("due").mock(res.sendfile).call(res, "./static/html/keyoverview.html").then(function(error) {
            if (error != null) {
      ...
      this.express.get("/:type", function(_this) {
        return function(req, res, next) {
          return require("due").mock(res.sendfile).call(res, "./static/html/" + req.params.type + "overview.html");
        };
      }(this));
    ...
    Overview.prototype.generateViews = function() {
      ...
      this.emit(
        "initStatusUpdate",
        "Started reading the keys from local file, requesting information about the key from redis and packing these information."
      );
    ...
    Overview.prototype._getKeySizeAndType = function(keys, last) {
      ...
      this.redis.multi(_commands).exec(function(_this) {
        return function(err, content) {
          ...
          _this.emit(
            "initStatusPercentUpdate",
            Math.floor(_keysRequested / _this._totalKeyAmount * 100)
          );
    ...
    Overview.prototype._diffKeysAndSummarize = function(collection, last) {
      ...
      if (last) {
        this.emit(
          "initStatusUpdate",
          "Finished getting the necessary key information from redis."
        );
    ...
    Overview.prototype._getMemberCount = function(keys, last) {
      ...
      this.redis.multi(_commands).exec(function(_this) {
        return function(err, count) {
          ...
          for (_index = _j = 0, _ref = count.length - 1;
            (0 <= _ref ? _j <= _ref : _j >= _ref); _index = (0 <= _ref ? ++_j : --_j)) {
    ...
    Overview.prototype._createOverview = function() {
      ...
      for (type in _ref) {
        ...
        if (this._templateDataParsed[type] == null) {
          require("due").mock(fs.unlink).call(fs, "./static/html/" + type + "overview.html").then(function(delerror) {
      ...
      if (Object.keys(this._templateDataParsed).length !== 0) {
        fs.readFile("./views/typeoverview.hbs", {
          ...
          return function(error, data) {
            ...
            _fn = function(k) {
              require("due").mock(fs.writeFile).call(fs, "./static/html/" + k + "overview.html", _template(v)).then(function() {
                if (_last === k && _fin) {
                  ...
                  if (_this._keysDeleted > 0) {
                    _this.emit(
                      "initStatusUpdate",
                      "" + _this._keysDeleted + " Keys were deleted / ignored during the generation!"
                    );
                  }
      ...
      if (Object.keys(this._templateData.key.types).length !== 0) {
        ...
        fs.readFile("./views/keyoverview.hbs", {
          ...
          return function(error, data) {
            ...
            require("due").mock(fs.writeFile).call(fs, "./static/html/keyoverview.html", _template(_keytemplatedata)).then(function() {
              _finCreating();
            });
          };
        }(this));
        ...
        exec(
          "cp ./views/keyoverview_empty.html ./static/html/keyoverview.html",
          function(_this) {
    ...
    Overview.prototype._parseDataForTemplate = function() {
      ...
      for (k in _ref) {
        ...
        _templateDataParsed[k] = {
          ...
          "avgsize": this._formatByte(Math.round(
            this._templateData.key.types[k].size / this._templateData.key.types[k].amount
          ))
        };
        ...
        for (i = _i = 0, _ref1 = this._templateData[k].size.length - 1;
          (0 <= _ref1 ? _i <= _ref1 : _i >= _ref1); i = (0 <= _ref1 ? ++_i : --_i)) {
          _templateDataParsed[k].types.push({
            ...
            "size_percent": (Math.round(
              this._templateData[k].size[i].size / this._templateData.key.types[k].size * 10000
            ) / 100).toFixed(2) + "%",
            ...
            "amount_percent": (Math.round(
              this._templateData[k].membercount[i].membercount / this._templateData[k].totalamount * 10000
            ) / 100).toFixed(2) + "%"
          });
        }
      }
    ...
    Overview.prototype._parseKeysForTemplate = function() {
      ...
      types.totalavg = this._insertThousendsPoints(this._formatByte(
        Math.round(this._templateData.key.totalsize / this._templateData.key.totalamount)
      ));
